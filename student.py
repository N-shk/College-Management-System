...
class StudentClass:
    def __init__(self, root):
        ...
        #=====Search Panel====
        Label(self.root, text="Select Class", font=("goudy old style", 18, "bold"), bg="white").place(x=780, y=100)
        ...
        self.txt_search_class.place(x=915, y=100, width=180)
        self.txt_search_class.current(0)
        Button(self.root, text="Search", font=("goudy old style", 18, "bold"), bg="#03a9f4", fg="white", cursor="hand2", command=self.search).place(x=1105, y=100, width=110, height=28)

        #=====Content====
        self.C_Frame = Frame(self.root, bd=2, relief=RIDGE)
        self.C_Frame.place(x=780, y=150, width=750, height=400)
        ...
        style.configure("Treeview", font=(14), rowheight=35)
        ...
        self.subjectTable.column("name",anchor=CENTER, width=120)
        self.subjectTable.column("email",anchor=CENTER, width=130)
        self.subjectTable.column("gender",anchor=CENTER, width=90)
        self.subjectTable.column("dob",anchor=CENTER, width=100)
        ...
        try:
            if self.var_roll.get() == "" or self.var_class.get() == "Select":
                ...
                if row is not None:
                    ...
                    cur.execute("INSERT INTO students (roll, student_name, student_class, email, dob, gender, contact, city, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (
                        self.var_roll.get(),
                        self.var_name.get(),
                        self.var_class.get(),
                        self.var_email.get(),
    ...
    def show(self):
        ...
        cur = con.cursor()
        try:
            cur.execute("SELECT student_class, roll, student_name, email, gender, dob, contact, city, address FROM students")
            rows = cur.fetchall()
        ...
        try:
            if self.var_roll.get() == "":
                ...
                if row is None:
                    ...
                    cur.execute("UPDATE students SET student_name=?, student_class=?, email=?, dob=?, gender=?, contact=?, city=?, address=? WHERE roll=?", (
                        self.var_name.get(),
                        self.var_class.get(),
                        self.var_email.get(),
                        self.var_dob.get(),
        ...
        try:
            if self.var_roll.get() == "":
                ...
                if row is None:
                    ...
                    if op:
                        cur.execute("DELETE FROM students WHERE roll=? AND student_class=?", (self.var_roll.get(), self.var_class.get()))
                        con.commit()
    ...
    def search(self):
        ...
        try:
            if self.var_search.get() == "Select":
                ...
                cur.execute("SELECT * FROM students WHERE student_class=?", (self.var_search.get(),))
                rows = cur.fetchall()
                if len(rows) != 0:
                    self.subjectTable.delete(*self.subjectTable.get_children())
                    for row in rows:
